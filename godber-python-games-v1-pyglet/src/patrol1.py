...
monster_sprite.update(x=6 * 64, y=3 * 64, scale=2.0)
monster_sprite.dx = 1.0
monster_sprite.t = 0.0  # initial value

def monster_update(dt):
    monster_sprite.t = monster_sprite.t + dt
    monster_sprite.x += 3 * math.cos(monster_sprite.dx * monster_sprite.t)
    monster_sprite.y += 3 * math.sin(monster_sprite.dx * monster_sprite.t)
pyglet.clock.schedule_interval(monster_update, 1/60.0)
